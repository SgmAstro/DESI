...
def randoms(field='G9', survey='gama', density=1., zmin=0.039, zmax=0.263, dryrun=False, prefix='', seed=314, oversample=8, realz=0):
    ...
    if survey == 'gama':    
        ...
        if 'NERSC_HOST' in os.environ.keys():
            ...
            randoms = desi_randoms(int(field[1:]), _nrealisations)
            ...
            rpath = findfile(ftype='randoms', dryrun=False, field=field, survey=survey, prefix=None, realz=realz, oversample=oversample)
            randoms.write(rpath, format='fits', overwrite=True)
